    ...
    <div>
   		<div class="col-xs-12 no-padding">
   			<div class="box box-solid">
   				<form id="search_form_workOvertime_audit_list" action="" class="form-horizontal">
   				...
   				<div class="box-body">
   					<table id="workOvertimeAuditDatagrid"></table>
   					<div id="workOvertimeAuditDatagridpager"></div>
   				</div>
   			</div>
   		</div>
   	</div>
   	<script type="text/javascript">
   		var auditWorkOvertimeGrid;
   		var auditWorkOvertimeDialog;
   		$(function() {
   			
   			$("#search_form_workOvertime_audit_list").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
			initSelect_workOvertimeAudit("search_form_workOvertime_audit_list");
			$("#searchLeaveAuditList").click(function() {
   				$("#workOvertimeAuditDatagrid").setGridParam({
   					postData:parsePostData("search_form_workOvertime_audit_list")
   				}).trigger("reloadGrid");
   			});
   			
   			$("#resetSearchLeaveAuditList").click(function() {
   				$("#search_form_workOvertime_audit_list")[0].reset();
   				$("#search_form_workOvertime_audit_list select.select2").each(function() {
   					$(this).val("").trigger("change");
   				});
   			});
   			
   			auditWorkOvertimeGrid = $("#workOvertimeAuditDatagrid").jqGrid({
                url: getRoot() + "workflow/workOvertime/queryAuditPage.action",
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                colModel: [{
                	...
                	label: "操作", formatter: function(cellvalue, options, rowObject) {
                		var buttons = "";
                		buttons = buttons + createBtn("查看", "btn-info btn-xs", "fa-file-text-o", "viewAuditWorkOvertime("+rowObject.id+", "+rowObject.passBtn+", "+rowObject.rejectBtn+")");
                		if(true == rowObject.passBtn) {
                			buttons = buttons + createBtn("通过", "btn-success btn-xs", "fa-share", "auditWorkOvertime("+rowObject.id+", 0)");
                		}
                		if(true == rowObject.rejectBtn) {
                			buttons = buttons + createBtn("退回", "btn-danger btn-xs", "fa-reply", "auditWorkOvertime("+rowObject.id+", 1)");
                		}
   						return buttons;
                	}
                }],
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#workOvertimeAuditDatagridpager"
            });
   		});
   		
   		function viewAuditWorkOvertime(id, passBtn, rejectBtn) {
   			var passCss = "";
   			var rejectCss = "";
   			if(true != passBtn) {
   				passCss = "hidden";
   			...
   			auditWorkOvertimeDialog = BootstrapDialog.show({
			    title: "详情",
			    width: "50%",
			    type: BootstrapDialog.TYPE_DEFAULT,
			    message: $("<div></div>").load(getRoot() + "workflow/workOvertime/queryById.action?returnType=audit&workOvertime.id="+id),
			    draggable: true,
			    autodestroy: true,
			    ...
			    buttons: [{
			    	...
			    	action: function(dialog) {
			    		try {
			    			...
			    			commitAuditWorkOvertime(0, $form.serialize(), dialog, $button)
			    		} catch(ex) {
			    	...
			    	action: function(dialog) {
			    		try {
			    			var $button = this;
			    			var $form = dialog.getModalBody().find("form");
			    			commitAuditWorkOvertime(1, $form.serialize(), dialog, $button)
			    		} catch(ex) {
			    			dialog.enableButtons(true);
		    				dialog.setClosable(true);
			    			BootstrapDialog.danger("系统异常，请联系管理员！");
   		 ...
   		 * 审核方法
   		 */
   		function auditWorkOvertime(id, passType) {
   			if(!id) {
   				BootstrapDialog.danger("ID无效");
   				return false;
   			}
   			var param = "workOvertime.id="+id;
   			commitAuditWorkOvertime(passType, param);
   		}
   		 ...
   		 * @param $button 点击的按钮
   		 */
   		function commitAuditWorkOvertime(passType, params, dialog, $button) {
   			var tips = "";
   			if(passType == 0) {
   				tips = "通过";
   			} else {
   			...
   			BootstrapDialog.confirm({
	            ...
	            callback: function(y) {
	                ...
	                if(y) {
	   					$.ajax({
	   						url: getRoot() + "workflow/workOvertime/audit.action",
		    				...
		    				success: function(data) {
		    					...
		    					if(json.success) {
		    						...
		    						auditWorkOvertimeGrid.jqGrid().trigger("reloadGrid");
		    					} else {
   		...
   		function initSelect_workOvertimeAudit(id) {
   			try {
   	   				...
   	   				success: function(data) {
   	   					if(data) {
   	   						...
   	   						for(var key in json) {
   	   							...
   	   							initOptions_workOvertimeAudit(json[key], selector);
   	   						}
   	   					}
   	   				}
   	   			});
   		...
   		function initOptions_workOvertimeAudit(data, selector) {
