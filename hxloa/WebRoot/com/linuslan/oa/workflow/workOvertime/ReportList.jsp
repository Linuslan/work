    ...
    <div>
   		<div class="col-xs-12 no-padding">
   			<div class="box box-solid">
   				...
   				<form id="search_form_workOvertime_report_list" action="" class="form-horizontal">
   					<div class="box-body padding20">
						...
						<div class="form-group">
							...
							<div class="col-md-3 col-sm-6">
								<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="查询" id="searchLeaveReportList"><i class="fa fa-fw fa-search"></i>查询</button>
								<button type="button" class="btn btn-info btn-sm margin-left5" data-toggle="tooltip" title="重置" id="resetSearchLeaveReportList"><i class="fa fa-fw fa-undo"></i>重置</button>
							</div>
						</div>
   					</div>
   				</form>
   				...
   				<div class="box-body">
   					<table id="workOvertimeReportDatagrid"></table>
   					<div id="workOvertimeReportDatagridpager"></div>
   				</div>
   			</div>
   		</div>
   	</div>
   	<script type="text/javascript">
   		var reportWorkOvertimeGrid;
   		var reportWorkOvertimeDialog;
   		$(function() {
   			
   			$("#search_form_workOvertime_report_list").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
			initSelect_workOvertimeReport("search_form_workOvertime_report_list");
			$("#searchLeaveReportList").click(function() {
   				$("#workOvertimeReportDatagrid").setGridParam({
   					postData:parsePostData("search_form_workOvertime_report_list")
   				}).trigger("reloadGrid");
   			});
   			
   			$("#resetSearchLeaveReportList").click(function() {
   				$("#search_form_workOvertime_report_list")[0].reset();
   				$("#search_form_workOvertime_report_list select.select2").each(function() {
   					$(this).val("").trigger("change");
   				});
   			});
   			
   			reportWorkOvertimeGrid = $("#workOvertimeReportDatagrid").jqGrid({
                url: getRoot() + "workflow/workOvertime/queryReportPage.action",
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                colModel: [{
                	...
                	label: "操作", formatter: function(cellvalue, options, rowObject) {
                		var buttons = "";
                		buttons = buttons + createBtn("查看", "btn-info btn-xs", "fa-file-text-o", "viewReportWorkOvertime("+rowObject.id+", "+rowObject.passBtn+", "+rowObject.rejectBtn+")");
   						return buttons;
                	}
                }],
				viewrecords: true,
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#workOvertimeReportDatagridpager"
            });
   		});
   		
   		function viewReportWorkOvertime(id, passBtn, rejectBtn) {
   			reportWorkOvertimeDialog = BootstrapDialog.show({
			    title: "详情",
			    width: "50%",
   		...
   		function initSelect_workOvertimeReport(id) {
   			try {
   	   				...
   	   				success: function(data) {
   	   					if(data) {
   	   						...
   	   						for(var key in json) {
   	   							...
   	   							initOptions_workOvertimeReport(json[key], selector);
   	   						}
   	   					}
   	   				}
   	   			});
   		...
   		function initOptions_workOvertimeReport(data, selector) {
