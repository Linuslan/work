    ...
    <div>
   		<div class="col-xs-12 no-padding">
   			<div class="box box-solid">
   				<form id="search_form_purchaseReq_audited_list" action="" class="form-horizontal">
   					<div class="box-body padding20">
						...
						<div class="form-group">
							...
							<div class="col-md-3 col-sm-6">
								<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="查询" id="searchPurchaseReqAuditedList"><i class="fa fa-fw fa-search"></i>查询</button>
								<button type="button" class="btn btn-info btn-sm margin-left5" data-toggle="tooltip" title="重置" id="resetSearchPurchaseReqAuditedList"><i class="fa fa-fw fa-undo"></i>重置</button>
							</div>
						</div>
   				...
   				<div class="box-body">
   					<table id="purchaseReqAuditedDatagrid"></table>
   					<div id="purchaseReqAuditedDatagridpager"></div>
   				</div>
   			</div>
   		</div>
   	</div>
   	<script type="text/javascript">
   		var auditedPurchaseReqGrid;
   		var auditedPurchaseReqDialog;
   		$(function() {
   			
   			$("#search_form_purchaseReq_audited_list").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
			initSelect_purchaseReqAudited("search_form_purchaseReq_audited_list");
			$("#searchPurchaseReqAuditedList").click(function() {
   				$("#purchaseReqAuditedDatagrid").setGridParam({
   					postData:parsePostData("search_form_purchaseReq_audited_list")
   				}).trigger("reloadGrid");
   			});
   			
   			$("#resetSearchPurchaseReqAuditedList").click(function() {
   				$("#search_form_purchaseReq_audited_list")[0].reset();
   				$("#search_form_purchaseReq_audited_list select.select2").each(function() {
   					$(this).val("").trigger("change");
   				});
   			});
   			
   			auditedPurchaseReqGrid = $("#purchaseReqAuditedDatagrid").jqGrid({
                url: getRoot() + "workflow/purchaseReq/queryAuditedPage.action",
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                colModel: [{
                	...
                	label: "操作", formatter: function(cellvalue, options, rowObject) {
                		var buttons = "";
                		buttons = buttons + createBtn("查看", "btn-info btn-xs", "fa-file-text-o", "viewAuditedPurchaseReq("+rowObject.id+", "+rowObject.passBtn+", "+rowObject.rejectBtn+")");
   						return buttons;
                	}
                }],
				viewrecords: true,
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#purchaseReqAuditedDatagridpager"
            });
   		});
   		
   		function viewAuditedPurchaseReq(id, passBtn, rejectBtn) {
   			auditedPurchaseReqDialog = BootstrapDialog.show({
			    title: "详情",
			    width: "60%",
   		...
   		function initSelect_purchaseReqAudited(id) {
   			try {
   	   				...
   	   				success: function(data) {
   	   					if(data) {
   	   						...
   	   						for(var key in json) {
   	   							...
   	   							initOptions_purchaseReqAudited(json[key], selector);
   	   						}
   	   					}
   	   				}
   	   			});
   		...
   		function initOptions_purchaseReqAudited(data, selector) {
