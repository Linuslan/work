...
<html>
  ...
  <body>
   	<form id="editArticleForm" action="" class="form-horizontal">
   		<input id="editArticleId" class="articleId" name="article.id" type="hidden" value="${article.id }" />
    	...
    	<div class="box-body padding-bottom5 bottom-dotted-border" >
    		<div class="form-group">
				...
				<div class="col-md-2 col-sm-8 no-padding">
					<input name="article.name" type="text" value="${article.name }" class="form-control required" id="text">
				</div>
				<label for="text" class="col-md-2 col-sm-4 control-label">商品编号：</label>
				<div class="col-md-2 col-sm-8 totalScore no-padding">
					<input name="article.serialNo" type="text" value="${article.serialNo }" class="form-control required" id="text">
				</div>
				<label for="text" class="col-md-2 col-sm-4 control-label">单位：</label>
				<div class="col-md-2 col-sm-8 totalScore no-padding">
					<input name="article.unit" type="text" value="${article.unit }" class="form-control required" id="text">
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-md-2 col-sm-8 totalScore no-padding">
					<select class="select2" name="article.companyId" style="width: 100%">
						...
						<c:forEach items="${companys }" var="company">
							<option value="${company.id }" ${company.id == article.companyId ? "selected='selected'" : "" }>${company.name }</option>
						</c:forEach>
					</select>
				</div>
				<label for="text" class="col-md-2 col-sm-4 control-label">排序号：</label>
				<div class="col-md-2 col-sm-8 totalScore no-padding">
					<input name="article.orderNo" type="number" value="${article.orderNo }" class="form-control required" id="text">
				</div>
			</div>
    	</div>
    	<div class="toolbar with-border">
			<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="新增规格" id="addFormat_edit"><i class="fa fa-fw fa-plus"></i>新增</button>
		</div>
    	<div class="box-body">
    		<table id="formatDatagrid_edit"></table>
    	</div>
   	</form>
   	<script type="text/javascript">
   		$(function() {
   			$("#editArticleForm").find(".select2").select2();
   			$("#editArticleForm").find(".date").datepicker({format: "yyyy-mm", language: "zh-CN"});
			//需要延时，否则加载时，设置宽度会出异常，宽度不能自适应
            setTimeout("generateFormatgGrid_edit()", 0);
            $("#addFormat_edit").click(function() {
            	var rowId = Math.random();
            	$("#formatDatagrid_edit").addRowData(rowId, {
            		id: "",
            		name: "",
            		price: "",
            		orderNo: "",
   		...
   		var rowDatas;
   		function generateFormatgGrid_edit() {
   			var id = $("#editArticleForm").find("input.articleId").val();
   			$("#formatDatagrid_edit").jqGrid({
                ...
                colModel: [{
                	...
                	label: "操作", name: "operationCell", align: "center", width: 70, formatter: function(cellvalue, options, rowObject) {
                		...
                		var rowId = options.rowId;
               			buttons = buttons + createBtn("删除", "btn-danger btn-xs", "fa-trash-o", "delFormat_edit(\""+rowObject.id+"\", \""+rowId+"\")");
   						return buttons;
                	}
                }],
                ondblClickRow: function(id){
                	if(id && id!==formatLastSel2){
                		...
                		jQuery("#formatDatagrid_edit").editRow(id,true);
                		formatLastSel2=id;
                	}
			    },
				viewrecords: true,
		...
		function delFormat_edit(id, rowId) {
	                		...
	                		$.ajax({
			    				...
			    				success: function(data) {
			    					...
			    					if(json.success) {
			    						...
			    						$("#formatDatagrid_edit").delRowData(rowId);
			    					} else {
