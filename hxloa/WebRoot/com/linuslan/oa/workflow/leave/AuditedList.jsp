    ...
    <div>
   		<div class="col-xs-12 no-padding">
   			<div class="box box-solid">
   				<form id="search_form_leave_audited_list" action="" class="form-horizontal">
   				...
   				<div class="box-body">
   					<table id="leaveAuditedDatagrid"></table>
   					<div id="leaveAuditedDatagridpager"></div>
   				</div>
   			</div>
   		</div>
   	</div>
   	<script type="text/javascript">
   		var auditedLeaveGrid;
   		var auditedLeaveDialog;
   		$(function() {
   			
			$("#search_form_leave_audited_list").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
			initSelect_leaveAudited("search_form_leave_audited_list");
			$("#searchLeaveAuditedList").click(function() {
   				$("#leaveAuditedDatagrid").setGridParam({
   					postData:parsePostData("search_form_leave_audited_list")
   				}).trigger("reloadGrid");
   			});
   			
   			$("#resetSearchLeaveAuditedList").click(function() {
   				$("#search_form_leave_audited_list")[0].reset();
   				$("#search_form_leave_audited_list select.select2").each(function() {
   					$(this).val("").trigger("change");
   				});
   			});
   			auditedLeaveGrid = $("#leaveAuditedDatagrid").jqGrid({
                url: getRoot() + "workflow/leave/queryAuditedPage.action",
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                colModel: [{
                	...
                	label: "操作", formatter: function(cellvalue, options, rowObject) {
                		var buttons = "";
                		buttons = buttons + createBtn("查看", "btn-info btn-xs", "fa-file-text-o", "viewAuditedLeave("+rowObject.id+", "+rowObject.passBtn+", "+rowObject.rejectBtn+")");
   						return buttons;
                	}
                }],
				viewrecords: true,
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#leaveAuditedDatagridpager"
            });
   		});
   		
   		function viewAuditedLeave(id, passBtn, rejectBtn) {
   			auditedLeaveDialog = BootstrapDialog.show({
			    title: "详情",
			    width: "50%",
   		...
   		function initSelect_leaveAudited(id) {
   			try {
   	   				...
   	   				success: function(data) {
   	   					if(data) {
   	   						...
   	   						for(var key in json) {
   	   							...
   	   							initOptions_leaveAudited(json[key], selector);
   	   						}
   	   					}
   	   				}
   	   			});
   		...
   		function initOptions_leaveAudited(data, selector) {
