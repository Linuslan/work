    ...
    <div>
   		<div class="col-xs-12 no-padding">
   			<div class="box box-solid">
   				<form id="search_form_leave_report_list" action="" class="form-horizontal">
   				...
   				<div class="box-body">
   					<table id="leaveReportDatagrid"></table>
   					<div id="leaveReportDatagridpager"></div>
   				</div>
   			</div>
   		</div>
   	</div>
   	<script type="text/javascript">
   		var reportLeaveGrid;
   		var reportLeaveDialog;
   		$(function() {
   			
			$("#search_form_leave_report_list").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
			initSelect_leaveReport("search_form_leave_report_list");
			$("#searchLeaveReportList").click(function() {
   				$("#leaveReportDatagrid").setGridParam({
   					postData:parsePostData("search_form_leave_report_list")
   				}).trigger("reloadGrid");
   			});
   			
   			$("#resetSearchLeaveReportList").click(function() {
   				$("#search_form_leave_report_list")[0].reset();
   				$("#search_form_leave_report_list select.select2").each(function() {
   					$(this).val("").trigger("change");
   				});
   			});
   			reportLeaveGrid = $("#leaveReportDatagrid").jqGrid({
                url: getRoot() + "workflow/leave/queryReportPage.action",
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                colModel: [{
                	...
                	label: "操作", formatter: function(cellvalue, options, rowObject) {
                		var buttons = "";
                		buttons = buttons + createBtn("查看", "btn-info btn-xs", "fa-file-text-o", "viewReportLeave("+rowObject.id+", "+rowObject.passBtn+", "+rowObject.rejectBtn+")");
   						return buttons;
                	}
                }],
				viewrecords: true,
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#leaveReportDatagridpager"
            });
   		});
   		
   		function viewReportLeave(id, passBtn, rejectBtn) {
   			reportLeaveDialog = BootstrapDialog.show({
			    title: "详情",
			    width: "50%",
   		...
   		function initSelect_leaveReport(id) {
   			try {
   	   				...
   	   				success: function(data) {
   	   					if(data) {
   	   						...
   	   						for(var key in json) {
   	   							...
   	   							initOptions_leaveReport(json[key], selector);
   	   						}
   	   					}
   	   				}
   	   			});
   		...
   		function initOptions_leaveReport(data, selector) {
