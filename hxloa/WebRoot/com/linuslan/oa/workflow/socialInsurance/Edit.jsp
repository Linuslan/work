...
<html>
  ...
  <body>
   	<form id="editSocialInsuranceForm" action="<%=basePath %>/workflow/socialInsurance/update.action" class="form-horizontal" method="POST" enctype="multipart/form-data">
   		<input id="editSocialInsuranceId" class="socialInsuranceId" name="socialInsurance.id" type="hidden" value="${socialInsurance.id }" />
    	<div class="box-body padding-bottom5 bottom-dotted-border" >
    		<div class="form-group">
				...
				<div class="col-md-3 col-sm-8 no-padding">
					<div class="input-group">
						...
						<input readonly="readonly" name="date" type="text" value="${socialInsurance.year }-${socialInsurance.month}" class="form-control pull-right date">
					</div>
				</div>
			...
			<div class="form-group">
				...
				<div class="col-md-9 col-sm-8 no-padding">
					<textarea name="socialInsurance.remark" class="form-control" rows="3" placeholder="请输入">${socialInsurance.remark }</textarea>
				</div>
			</div>
    	</div>
    	<div class="toolbar with-border">
			<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="新增社保项目" id="addSocialInsuranceContent_edit"><i class="fa fa-fw fa-plus"></i>新增</button>
			<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="删除" id="delSocialInsuranceContent_edit"><i class="fa fa-fw fa-trash"></i>删除</button>
		</div>
    	<div class="box-body" style="overflow-x: auto;">
    		<table id="socialInsuranceContentDatagrid_edit"></table>
    	</div>
   	</form>
   	<script type="text/javascript">
   		$(function() {
   			$("#editSocialInsuranceForm").find(".date").datepicker({format: "yyyy-mm", language: "zh-CN"});
   			
			//需要延时，否则加载时，设置宽度会出异常，宽度不能自适应
			setTimeout("generateEditSocialInsuranceContentGrid()", 0);
            
            $("#addSocialInsuranceContent_edit").click(function() {
            	if(socialInsuranceLastsel2){
               		jQuery("#socialInsuranceContentDatagrid_edit").saveRow(socialInsuranceLastsel2, false, "clientArray");
               	}
            	
            	var rowId = Math.random();
            	
            	$("#socialInsuranceContentDatagrid_edit").addRowData(rowId, {
            		orderNo: "",
            		userName: "",
            		idNo: "",
            		company: "",
   		...
   		var rowDatas;
   		function generateEditSocialInsuranceContentGrid() {
   			var id = $("#editSocialInsuranceForm").find("input.socialInsuranceId").val();
   			$("#socialInsuranceContentDatagrid_edit").jqGrid({
                ...
                colModel: [{
                	...
                	label: "个人合计", name: "userSum", width: 110, align: "center", editable: true, edittype: "custom",
                	editoptions: {
                		custom_element: createNumberBox,
                		custom_value: operateNumberValue,
                		name: "userSum"
                ...
                ondblClickRow: function(id){
                	if(id && id!==socialInsuranceLastsel2){
                		jQuery("#socialInsuranceContentDatagrid_edit").saveRow(socialInsuranceLastsel2, false, "clientArray");
                		jQuery("#socialInsuranceContentDatagrid_edit").editRow(id,true);
   			...
   			$("#socialInsuranceContentDatagrid_edit").jqGrid("setGroupHeaders", {
   			...
   			$("#delSocialInsuranceContent_edit").click(function() {
   				var selectedIds = $("#socialInsuranceContentDatagrid_edit").getGridParam("selarrrow");
   				
   				delBatchSocialInsuranceContent_edit(selectedIds);
   			});
   		}
   		 ...
   		 * 批量删除
   		 */
   		function delBatchSocialInsuranceContent_edit(ids) {
   			if(!ids && 0 >= ids.length) {
	                	...
	                	try {
		                	...
		                	for(var i = 0; i < ids.length; i ++) {
			            		...
			            		if(id && ""!=$.trim(id) && isNumber(id)) {
				            		...
				            		$("#socialInsuranceContentDatagrid_edit").delRowData(id);
				            	}
			            	}
		...
		function delEditSocialInsuranceContent(id, rowId) {
			BootstrapDialog.confirm({
	            title: "温馨提示",
	            message: "您确定删除吗？",
	            type: BootstrapDialog.TYPE_SUCCESS, // <-- Default value is BootstrapDialog.TYPE_PRIMARY
	                	...
	                	if(id && ""!=$.trim(id) && isNumber(id)) {
	                		$.ajax({
			    				...
			    				success: function(data) {
			    					...
			    					if(json.success) {
			    						BootstrapDialog.success(json.msg);
			    						$("#socialInsuranceContentDatagrid_edit").delRowData(rowId);
			    					} else {
			    						BootstrapDialog.danger(json.msg);
			    					}
			    				},
