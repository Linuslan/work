    ...
    <div>
   		<div class="col-xs-12 no-padding">
   			<div class="box box-solid">
   				<form id="search_form_sale_audit_list" action="" class="form-horizontal">
   					<div class="box-body padding20">
						...
						<div class="form-group">
							...
							<div class="col-md-3 col-sm-6">
								<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="查询" id="searchSaleAuditList"><i class="fa fa-fw fa-search"></i>查询</button>
								<button type="button" class="btn btn-info btn-sm margin-left5" data-toggle="tooltip" title="重置" id="resetSearchSaleAuditList"><i class="fa fa-fw fa-undo"></i>重置</button>
							</div>
						</div>
   				...
   				<div class="box-body">
   					<table id="saleAuditDatagrid"></table>
   					<div id="saleAuditDatagridpager"></div>
   				</div>
   			</div>
   		</div>
   	</div>
   	<script type="text/javascript">
   		var auditSaleGrid;
   		var auditSaleDialog;
   		var auditSaleLastSel;
   		$(function() {
   		
   			$("#search_form_sale_audit_list").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
			initSelect_saleAudit("search_form_sale_audit_list");
			$("#searchSaleAuditList").click(function() {
   				$("#saleAuditDatagrid").setGridParam({
   					postData:parsePostData("search_form_sale_audit_list")
   				}).trigger("reloadGrid");
   			});
   			
   			$("#resetSearchSaleAuditList").click(function() {
   				$("#search_form_sale_audit_list")[0].reset();
   				$("#search_form_sale_audit_list select.select2").each(function() {
   					$(this).val("").trigger("change");
   				});
   			});
   			
   			auditSaleGrid = $("#saleAuditDatagrid").jqGrid({
                url: getRoot() + "workflow/sale/queryAuditPage.action",
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                colModel: [{
                	...
                	label: "操作", formatter: function(cellvalue, options, rowObject) {
                		var buttons = "";
                		buttons = buttons + createBtn("查看", "btn-info btn-xs", "fa-file-text-o", "viewAuditSale("+rowObject.id+", "+rowObject.passBtn+", "+rowObject.rejectBtn+", \""+rowObject.flowStatus+"\")");
                		if(true == rowObject.passBtn) {
                			buttons = buttons + createBtn("通过", "btn-success btn-xs", "fa-share", "auditSale("+rowObject.id+", 0)");
                		}
                		if(true == rowObject.rejectBtn) {
                			buttons = buttons + createBtn("退回", "btn-danger btn-xs", "fa-reply", "auditSale("+rowObject.id+", 1)");
                		}
   						return buttons;
                	}
                }],
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#saleAuditDatagridpager"
            });
   		});
   		
   		function viewAuditSale(id, passBtn, rejectBtn, flowStatus) {
   			var passCss = "";
   			var rejectCss = "";
   			if(true != passBtn) {
   				passCss = "hidden";
   			...
   			auditSaleDialog = BootstrapDialog.show({
			    title: "详情",
			    width: "90%",
			    ...
			    buttons: [{
			    	...
			    	action: function(dialog) {
			    		try {
			    			...
			    			var $grid = dialog.getModalBody().find("form").find("table#saleContentDatagrid_checkoutAudit");
			    			if($grid && 0 < $grid.length) {
			    				jQuery("#saleContentDatagrid_checkoutAudit").jqGrid("saveRow", auditSaleLastSel, false, "clientArray");
			    				var contents = getAuditSaleContents($grid);
			    				commitAuditSale(0, $form.serialize()+"&"+contents.join("&"), dialog, $button);
			    			} else {
			    				commitAuditSale(0, $form.serialize(), dialog, $button);
			    			}
			    	...
			    	action: function(dialog) {
			    		try {
			    			var $button = this;
			    			var $form = dialog.getModalBody().find("form");
			    			commitAuditSale(1, $form.serialize(), dialog, $button)
			    		} catch(ex) {
			    			dialog.enableButtons(true);
		    				dialog.setClosable(true);
	   					...
	   					$.ajax({
		    				...
		    				success: function(data) {
		    					...
		    					if(json.success) {
		    						...
		    						auditSaleGrid.jqGrid().trigger("reloadGrid");
		    					} else {
   		...
   		function initSelect_saleAudit(id) {
   			try {
   	   				...
   	   				success: function(data) {
   	   					if(data) {
   	   						...
   	   						for(var key in json) {
   	   							...
   	   							initOptions_saleAudit(json[key], selector);
   	   						}
   	   					}
   	   				}
   	   			});
   		...
   		function initOptions_saleAudit(data, selector) {
