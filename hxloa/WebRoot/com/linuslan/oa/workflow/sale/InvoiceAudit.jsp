...
<html>
  ...
  <body>
   	<form id="invoiceAuditSaleForm" action="" class="form-horizontal">
   		<input id="invoiceAuditSaleId" class="saleId" name="sale.id" type="hidden" value="${sale.id }" />
   		<div class="nav-tabs-top-border">
   			<ul class="nav nav-tabs">
   				<li class="active">
   					<a href="#sale_invoiceAudit" data-toggle="tab">详情</a>
   				</li>
   				<li>
   					<a href="#sale_invoice_invoiceAudit" data-toggle="tab">开票信息</a>
   				</li>
   				<li>
   					<a href="#sale_invoiceAuditLog_invoiceAudit" data-toggle="tab">审核记录</a>
   				</li>
   			</ul>
   			<div class="tab-content">
   				<div class="active tab-pane" id="sale_invoiceAudit">
							...
							<div class="form-group">
								...
								<div class="col-md-2 col-sm-8 no-padding">
									<div class="input-group">
										...
										<input value="<fmt:formatDate value="${sale.confirmInvoiceDate }" pattern="yyyy-MM-dd"/>" class="form-control pull-right date showText" readonly="readonly" name="sale.confirmInvoiceDate" />
									</div>
								</div>
								<label for="departmentId" class="col-md-2-sm col-sm-4 control-label">确认人：</label>
								<div class="col-md-2 col-sm-8 left-label">
									${sessionScope.loginUser.name }
									<input type="hidden" name="sale.invoiceUserId" value="${sessionScope.loginUser.id }" />
								</div>
							</div>
						</c:if>
			    	</div>
			    	<div class="box-body" style="overflow-x: auto;">
			    		<table id="saleContentDatagrid_invoiceAudit"></table>
			    	</div>
			    </div>
			    <div class="tab-pane" id="sale_invoice_invoiceAudit">
			    ...
			    <div class="tab-pane" id="sale_invoiceAuditLog_invoiceAudit">
			    	<div class="box box-solid">
		   				<div class="box-body">
							...
							<div class="col-md-12 col-sm-12 no-padding">
			   					<table id="saleAuditorlogsDatagrid_invoiceAudit"></table>
	   							<div id="saleAuditorlogsDatagridPager_invoiceAudit"></div>
   							</div>
		   				</div>
		   			</div>
			    </div>
   	...
   	<script type="text/javascript">
   		$(function() {
   			$("#invoiceAuditSaleForm").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
            //需要延时，否则加载时，设置宽度会出异常，宽度不能自适应
			setTimeout("buildSaleContentGrid_invoiceAudit()", 0);
			setTimeout("buildSaleLogGrid_invoiceAudit()", 0);
   		});
   		
   		function buildSaleLogGrid_invoiceAudit() {
   			$("#saleAuditorlogsDatagrid_invoiceAudit").jqGrid({
                url: getRoot() + "workflow/auditlog/queryOpinionPage.action?wfType=sale&wfId="+$("#invoiceAuditSaleId").val(),
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#saleAuditorlogsDatagridPager_invoiceAudit"
            });
   		}
   		
   		function buildSaleContentGrid_invoiceAudit() {
   			var saleId = $("#invoiceAuditSaleForm").find("input[type=hidden][name='sale.id']").val();
   			$("#saleContentDatagrid_invoiceAudit").jqGrid({
                mtype: "POST",
