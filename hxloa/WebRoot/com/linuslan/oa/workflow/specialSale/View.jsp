...
<html>
  ...
  <body>
   	<form id="auditedSpecialSaleForm" action="" class="form-horizontal">
   		<input id="auditedSpecialSaleId" class="specialSaleId" name="specialSale.id" type="hidden" value="${specialSale.id }" />
   		<div class="nav-tabs-top-border">
   			<ul class="nav nav-tabs">
   				<li class="active">
   					<a href="#specialSale_audited" data-toggle="tab">详情</a>
   				</li>
   				<li>
   					<a href="#specialSale_auditLog_audited" data-toggle="tab">审核记录</a>
   				</li>
   			</ul>
   			<div class="tab-content">
   				<div class="active tab-pane" id="specialSale_audited">
			    	<div class="box-body padding-bottom5 bottom-dotted-border" >
						...
						<div class="form-group">
							...
							<div class="col-md-2 col-sm-8 left-label">
								<c:choose>
									<c:when test="${specialSale.checkoutStatus == 1 }">
										部分出库
									</c:when>
									<c:when test="${specialSale.checkoutStatus == 2 }">
										已出库
									</c:when>
									<c:otherwise>
										未出库
							...
							<div class="col-md-8 col-sm-8 left-label">
								${specialSale.stockmanInfo }
							</div>
						</div>
						<div class="form-group">
			    	...
			    	<div class="box-body" style="overflow-x: auto;">
			    		<table id="specialSaleContentDatagrid_audited"></table>
			    	</div>
			    </div>
			    <div class="tab-pane" id="specialSale_auditLog_audited">
			    	<div class="box box-solid">
		   				<div class="box-body">
							<div class="col-md-12 col-sm-12 no-padding">
			   					<table id="specialSaleAuditorlogsDatagrid_audited"></table>
	   							<div id="specialSaleAuditorlogsDatagridPager_audited"></div>
   							</div>
		   				</div>
		   			</div>
			    </div>
   	...
   	<script type="text/javascript">
   		$(function() {
			...
			setTimeout("buildSpecialSaleContentGrid_audited()", 0);
			setTimeout("buildSpecialSaleLogGrid_audited()", 0);
   		});
   		
   		function buildSpecialSaleLogGrid_audited() {
   			var specialSaleId = $("#auditedSpecialSaleForm").find("input[type=hidden][name='specialSale.id']").val();
   			$("#specialSaleAuditorlogsDatagrid_audited").jqGrid({
                url: getRoot() + "workflow/auditlog/queryOpinionPage.action?wfType=specialSale&wfId="+sepcialSaleId,
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#specialSaleAuditorlogsDatagridPager_audited"
            });
   		}
   		
   		function buildSpecialSaleContentGrid_audited() {
   			var specialSaleId = $("#auditedSpecialSaleForm").find("input[type=hidden][name='specialSale.id']").val();
   			$("#specialSaleContentDatagrid_audited").jqGrid({
                mtype: "POST",
