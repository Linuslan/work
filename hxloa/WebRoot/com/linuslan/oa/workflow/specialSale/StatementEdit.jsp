				...
				<table style="width: 100%;">
					<tr>
						...
						<td class="align-left" style="width: 15%">
							<select name="specialSale.accountId" class="form-control" style="width: 150px;" onchange="setAccount_specialSale_statement(this)">
								<option value="">请选择</option>
							<c:forEach items="${accounts }" var="account">
								<option value="${account.id }" ${specialSale.accountId == account.id ? "selected='selected'" : "" }
									bankNo="${account.bankNo }" bank="${account.bankName }">${account.receiver }</option>
							</c:forEach>
							</select>
						</td>
						<td class="align-right" style="width: 10%">账号：</td>
						<td style="width: 15%" class="bankNo">
							<input type="hidden" name="specialSale.bankNo" value="${specialSale.bankNo }" />
							<span>${specialSale.bankNo }</span>
						</td>
						<td class="align-right" style="width: 10%">开户行：</td>
						<td style="width: 15%" class="bank">
							<input type="hidden" name="specialSale.bank" value="${specialSale.bank }" />
