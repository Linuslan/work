    ...
    <div>
   		<div class="col-xs-12 no-padding">
   			<div class="box box-solid">
   				<form id="search_form_specialSale_list" action="" class="form-horizontal">
   					<div class="box-body padding20">
						...
						<div class="form-group">
							...
							<div class="col-md-3 col-sm-6">
								<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="查询" id="searchSpecialSaleList"><i class="fa fa-fw fa-search"></i>查询</button>
								<button type="button" class="btn btn-info btn-sm margin-left5" data-toggle="tooltip" title="重置" id="resetSearchSpecialSaleList"><i class="fa fa-fw fa-undo"></i>重置</button>
							</div>
						</div>
   				...
   				<div class="toolbar with-border">
   					<button class="btn btn-info btn-sm" data-toggle="tooltip" title="销售订单申请" id="addSpecialSale"><i class="fa fa-fw fa-plus"></i>新增</button>
   				</div>
   				<div class="box-body">
   					<table id="specialSaledatagrid"></table>
   					<div id="specialSaledatagridpager"></div>
   				</div>
   			</div>
   		</div>
   	</div>
   	<script type="text/javascript">
   		var specialSaleGrid;
   		var specialSaleDialog;
   		var specialSaleLastSel2;
   		$(function() {
   			
   			$("#search_form_specialSale_list").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
			initSelect_specialSale("search_form_specialSale_list");
			$("#searchSpecialSaleList").click(function() {
   				$("#specialSaledatagrid").setGridParam({
   					postData:parsePostData("search_form_specialSale_list")
   				}).trigger("reloadGrid");
   			});
   			
   			$("#resetSearchSpecialSaleList").click(function() {
   				$("#search_form_specialSale_list")[0].reset();
   				$("#search_form_specialSale_list select.select2").each(function() {
   					$(this).val("").trigger("change");
   				});
   			});
   			
   			specialSaleGrid = $("#specialSaledatagrid").jqGrid({
                url: getRoot() + "workflow/specialSale/queryPage.action",
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                colModel: [{
                	...
                	label: "操作", formatter: function(cellvalue, options, rowObject) {
                		var buttons = "";
                		buttons = buttons + createBtn("查看", "btn-info btn-xs", "fa-file-text-o", "viewSpecialSale("+rowObject.id+")");
                		if(rowObject.status != 0 && rowObject.status != 1 && rowObject.status != 2) {
                			buttons = buttons + createBtn("结算单", "btn-info btn-xs", "fa-file-text-o", "viewStatementSpecialSale("+rowObject.id+")");
                		}
                		
                		if(rowObject.allFlowStatus.indexOf("7") || rowObject.allFlowStatus.indexOf("8") || rowObject.status == 4) {
                			buttons = buttons + createBtn("出货单", "btn-info btn-xs", "fa-file-text-o", "viewCheckoutSpecialSale("+rowObject.id+")");
                		}
                		
                		if(rowObject.status == 0 || rowObject.status == 1 || rowObject.status == 2) {
                			buttons = buttons + createBtn("修改", "btn-info btn-xs", "fa-pencil", "editSpecialSale("+rowObject.id+")");
                			buttons = buttons + createBtn("提交", "btn-success btn-xs", "fa-share", "commitSpecialSale("+rowObject.id+")");
                			buttons = buttons + createBtn("删除", "btn-danger btn-xs", "fa-trash-o", "delSpecialSale("+rowObject.id+")");
                		}
   						return buttons;
                	}
                }],
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#specialSaledatagridpager"
            });
   			$("#addSpecialSale").click(function() {
   				specialSaleDialog = BootstrapDialog.show({
				    title: "新增申请",
				    width: "90%",
				    type: BootstrapDialog.TYPE_DEFAULT,
				    message: $("<div></div>").load(getRoot() + "workflow/specialSale/queryById.action?returnType=add"),
				    draggable: true,
				    autodestroy: true,
				    	...
				    	action: function(dialog) {
				    		try {
				    			...
				    			if(specialSaleLastSel2) {
				    				...
				    				jQuery("#specialSaleContentDatagrid_add").jqGrid("saveRow", specialSaleLastSel2, false, "clientArray");
					    			specialSaleLastSel2 = undefined;
				    			}
				    			
				    			var contents = getSpecialSaleContents($grid);
				    			if(!contents) {
				    				return false;
				    			}
				    			...
				    			var $button = this;
				    			$.ajax({
				    				url: getRoot() + "workflow/specialSale/add.action",
				    				...
				    				success: function(data) {
				    					...
				    					if(json.success) {
				    						...
				    						dialog.close();
				    						specialSaleGrid.jqGrid().trigger("reloadGrid");
				    					} else {
				    			...
				    			BootstrapDialog.confirm({
						            ...
						            callback: function(y) {
						                ...
						                if(y) {
							    			...
							    			$.ajax({
							    				url: getRoot() + "workflow/specialSale/commit.action?type=update",
							    				data: form.serialize()+"&"+contents.join("&"),
							    				type: "POST",
							    				success: function(data) {
							    					var json = eval("("+data+")");
   		...
   		function editSpecialSale(id) {
   			specialSaleDialog = BootstrapDialog.show({
			    title: "编辑销售订单",
			    width: "90%",
			    type: BootstrapDialog.TYPE_DEFAULT,
			    message: $("<div></div>").load(getRoot() + "workflow/specialSale/queryById.action?returnType=edit&specialSale.id="+id),
			    draggable: true,
			    autodestroy: true,
			    closeByBackdrop: false,
			    autospin: true,
			    ...
			    buttons: [{
			    	...
			    	action: function(dialog) {
			    		try {
			    			var $grid = dialog.getModalBody().find("form").find("table#specialSaleContentDatagrid_edit");
			    			//将未保存的先保存
			    			if(specialSaleLastSel2) {
			    				countSpecialSale_edit();
			    				jQuery("#specialSaleContentDatagrid_edit").jqGrid("saveRow", specialSaleLastSel2, false, "clientArray");
			    				specialSaleLastSel2 = undefined;
			    			}
			    			...
			    			var $button = this;
			    			$.ajax({
			    				url: getRoot() + "workflow/specialSale/update.action",
			    				data: form.serialize()+"&"+contents.join("&"),
			    				type: "POST",
			    				success: function(data) {
			    					var json = eval("("+data+")");
   		...
   		function commitSpecialSale(id) {
   			BootstrapDialog.confirm({
	            ...
	            callback: function(y) {
	                ...
	                if(y) {
	   					$.ajax({
	   						url: getRoot() + "workflow/specialSale/commit.action?type=commit",
		    				data: "specialSale.id="+id,
		    				type: "POST",
		    				beforeSend: function() {
   		...
   		function delSpecialSale(id) {
   			BootstrapDialog.confirm({
	            ...
	            callback: function(y) {
	                ...
	                if(y) {
	   					$.ajax({
	   						url: getRoot() + "workflow/specialSale/del.action",
		    				data: "specialSale.id="+id,
		    				type: "POST",
		    				beforeSend: function() {
		    					//mask($("#userdatagrid").parent());
   		...
   		function viewSpecialSale(id) {
   			specialSaleDialog = BootstrapDialog.show({
			    title: "销售订单详情",
			    width: "90%",
			    type: BootstrapDialog.TYPE_DEFAULT,
			    message: $("<div></div>").load(getRoot() + "workflow/specialSale/queryById.action?returnType=view&specialSale.id="+id),
			    draggable: true,
			    autodestroy: true,
			    closeByBackdrop: false,
			    autospin: true,
   		...
   		function viewCheckoutSpecialSale(id) {
   			specialSaleDialog = BootstrapDialog.show({
			    title: "销售出货单详情",
			    width: "90%",
			    type: BootstrapDialog.TYPE_DEFAULT,
   		...
   		function viewStatementSpecialSale(id) {
   			specialSaleDialog = BootstrapDialog.show({
			    title: "销售结算单详情",
			    width: "90%",
			    type: BootstrapDialog.TYPE_DEFAULT,
   		...
   		function getSpecialSaleContents($grid) {
   			if(!$grid || 0 >= $grid.length) {
   				BootstrapDialog.error("获取销售订单项目异常");
   				return false;
   			}
   		...
   		function initSelect_specialSale(id) {
   			try {
   	   				...
   	   				success: function(data) {
   	   					if(data) {
   	   						...
   	   						for(var key in json) {
   	   							...
   	   							initOptions_specialSale(json[key], selector);
   	   						}
   	   					}
   	   				}
   	   			});
   		...
   		function initOptions_specialSale(data, selector) {
