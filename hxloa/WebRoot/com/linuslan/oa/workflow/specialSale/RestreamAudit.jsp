...
<html>
  ...
  <body>
   	<form id="restreamAuditSpecialSaleForm" action="" class="form-horizontal">
   		<input id="restreamAuditSpecialSaleId" class="specialSaleId" name="specialSale.id" type="hidden" value="${specialSale.id }" />
   		<div class="nav-tabs-top-border">
   			<ul class="nav nav-tabs">
   				<li class="active">
   					<a href="#specialSale_restreamAudit" data-toggle="tab">详情</a>
   				</li>
   				<li>
   					<a href="#specialSale_restreamAuditLog_restreamAudit" data-toggle="tab">审核记录</a>
   				</li>
   			</ul>
   			<div class="tab-content">
   				<div class="active tab-pane" id="specialSale_restreamAudit">
			    	<div class="box-body padding-bottom5 bottom-dotted-border" >
						...
						<div class="form-group">
							<label for="departmentId" class="col-md-1 col-sm-4 control-label">回款状态：</label>
							<div class="col-md-2 col-sm-8 left-label">
								<select name="specialSale.restreamStatus" class="form-control">
    								<option value="0" ${specialSale.restreamStatus == 0 ? "selected='selected'" : "" }>未回款</option>
	    							<option value="1" ${specialSale.restreamStatus == 1 ? "selected='selected'" : "" }>部分回款</option>
	    							<option value="2" ${specialSale.restreamStatus == 2 ? "selected='selected'" : "" }>已回款</option>
    							</select>
							</div>
							<label for="departmentId" class="col-md-1 col-sm-4 control-label">应收账款：</label>
							<div class="col-md-2 col-sm-8 left-label">
								<input name="specialSale.supposedMoney" type="number" value="0" class="form-control">
							</div>
							<label for="departmentId" class="col-md-1 col-sm-4 control-label">回款金额：</label>
							<div class="col-md-2 col-sm-8 left-label">
								<input name="specialSale.actualMoney" type="number" value="0" class="form-control">
							</div>
							<label for="departmentId" class="col-md-1 col-sm-4 control-label"></label>
							<div class="col-md-2 col-sm-8 left-label">
						...
						<div class="form-group">
							...
							<div class="col-md-11 col-sm-8 no-padding">
								<textarea class="form-control" name="specialSale.financeInfo">${specialSale.financeInfo }</textarea>
							</div>
						</div>
			    	</div>
			    	<div class="box-body" style="overflow-x: auto;">
			    		<table id="specialSaleContentDatagrid_restreamAudit"></table>
			    	</div>
			    </div>
			    <div class="tab-pane" id="specialSale_restreamAuditLog_restreamAudit">
			    	<div class="box box-solid">
		   				<div class="box-body">
							...
							<div class="col-md-12 col-sm-12 no-padding">
			   					<table id="specialSaleAuditorlogsDatagrid_restreamAudit"></table>
	   							<div id="specialSaleAuditorlogsDatagridPager_restreamAudit"></div>
   							</div>
		   				</div>
		   			</div>
			    </div>
   	...
   	<script type="text/javascript">
   		$(function() {
   			$("#restreamAuditSpecialSaleForm").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
            //需要延时，否则加载时，设置宽度会出异常，宽度不能自适应
			setTimeout("buildSpecialSaleContentGrid_restreamAudit()", 0);
			setTimeout("buildSpecialSaleLogGrid_restreamAudit()", 0);
   		});
   		
   		function buildSpecialSaleLogGrid_restreamAudit() {
   			$("#specialSaleAuditorlogsDatagrid_restreamAudit").jqGrid({
                url: getRoot() + "workflow/auditlog/queryOpinionPage.action?wfType=specialSale&wfId="+$("#restreamAuditSpecialSaleId").val(),
                mtype: "POST",
                shrinkToFit: true,
                autowidth: true,
                scrollrows: false,
                ...
                //width: "100%",
                rowNum: 20,
                pager: "#specialSaleAuditorlogsDatagridPager_restreamAudit"
            });
   		}
   		
   		function buildSpecialSaleContentGrid_restreamAudit() {
   			var specialSaleId = $("#restreamAuditSpecialSaleForm").find("input[type=hidden][name='specialSale.id']").val();
   			$("#specialSaleContentDatagrid_restreamAudit").jqGrid({
                mtype: "POST",
