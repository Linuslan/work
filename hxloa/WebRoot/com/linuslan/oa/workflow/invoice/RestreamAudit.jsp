...
<html>
  ...
  <body>
   	<form id="auditInvoiceRestreamAuditForm" action="" class="form-horizontal">
   		<input type="hidden" name="invoice.id" id="viewInvoiceId" value="${invoice.id }" />
   		<input type="hidden" name="invoice.confirmRestreamUserId" value="${sessionScope.loginUser.id }">
						...
						<div class="form-group">
							...
							<div class="col-md-3 col-sm-8 left-label">
								<select name="invoice.invoiceStatus" id="auditorInvoiceStatus" class="select2" style="width: 100%">
   									...
   									<option value="2" selected="selected">未回款</option>
    								<option value="1" ${invoice.invoiceStatus == 1 ? "selected='selected'" : "" }>部分回款</option>
    								<option value="0" ${invoice.invoiceStatus == 0 ? "selected='selected'" : "" }>已回款</option>
    								<option value="6" ${invoice.invoiceStatus == 6 ? "selected='selected'" : "" }>已开票作废</option>
   								</select>
							</div>
						</div>
						<div class="form-group">
							<label for="orderNo" class="col-md-2 col-sm-4 control-label">回款时间：</label>
							<div class="col-md-3 col-sm-8 left-label">
								<div class="input-group">
									<div class="input-group-addon">
										<i class="fa fa-calendar"></i>
									</div>
									<input name="invoice.confirmRestreamDate" readonly="readonly" class="form-control pull-right date" type="text" value="<%=new SimpleDateFormat("yyyy-MM-dd").format(new Date()) %>" />
								</div>
							</div>
							<div class="col-md-1 col-sm-12 left-label"></div>
							<label for="text" class="col-md-2 col-sm-4 control-label">实回款金额：</label>
							<div class="col-md-3 col-sm-8 left-label">
								<input name="invoice.actualMoney" type="number" class="form-control" value="0">
							</div>
						</div>
						<div class="form-group">
							<label class="col-md-2 col-sm-4 control-label">财务备注：</label>
							<div class="col-md-10 col-sm-8 left-label">
								<textarea name="invoice.financeInfo" class="form-control" rows="3" placeholder="请输入">${invoice.financeInfo }</textarea>
							</div>
						</div>
						<div class="form-group">
   	...
   	<script type="text/javascript">
   		$(function() {
   			$("#auditInvoiceRestreamAuditForm").find(".select2").select2();
   			$("#auditInvoiceRestreamAuditForm").find(".date").datepicker({format: "yyyy-mm-dd", language: "zh-CN"});
