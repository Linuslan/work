...
<html>
  ...
  <body>
   	<form id="editHealthInsuranceForm" action="<%=basePath %>/workflow/healthInsurance/update.action" class="form-horizontal" method="POST" enctype="multipart/form-data">
   		<input id="editHealthInsuranceId" class="healthInsuranceId" name="healthInsurance.id" type="hidden" value="${healthInsurance.id }" />
    	<div class="box-body padding-bottom5 bottom-dotted-border" >
    		<div class="form-group">
				...
				<div class="col-md-3 col-sm-8 no-padding">
					<div class="input-group">
						...
						<input readonly="readonly" name="date" type="text" value="${healthInsurance.year }-${healthInsurance.month}" class="form-control pull-right date">
					</div>
				</div>
			...
			<div class="form-group">
				...
				<div class="col-md-9 col-sm-8 no-padding">
					<textarea name="healthInsurance.remark" class="form-control" rows="3" placeholder="请输入">${healthInsurance.remark }</textarea>
				</div>
			</div>
    	</div>
    	<div class="toolbar with-border">
			<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="新增医保项目" id="addHealthInsuranceContent_edit"><i class="fa fa-fw fa-plus"></i>新增</button>
			<button type="button" class="btn btn-info btn-sm" data-toggle="tooltip" title="删除" id="delHealthInsuranceContent_edit"><i class="fa fa-fw fa-trash"></i>删除</button>
		</div>
    	<div class="box-body">
    		<table id="healthInsuranceContentDatagrid_edit"></table>
    	</div>
   	</form>
   	<script type="text/javascript">
   		$(function() {
   			$("#editHealthInsuranceForm").find(".date").datepicker({format: "yyyy-mm", language: "zh-CN"});
   			
			//需要延时，否则加载时，设置宽度会出异常，宽度不能自适应
			setTimeout("generateEditHealthInsuranceContentGrid()", 0);
            
            $("#addHealthInsuranceContent_edit").click(function() {
            	if(healthInsuranceLastsel2){
               		jQuery("#healthInsuranceContentDatagrid_edit").saveRow(healthInsuranceLastsel2, false, "clientArray");
               	}
            	
            	var rowId = Math.random();
            	
            	$("#healthInsuranceContentDatagrid_edit").addRowData(rowId, {
            		orderNo: "",
            		userName: "",
            		idNo: "",
            		company: "",
   		...
   		var rowDatas;
   		function generateEditHealthInsuranceContentGrid() {
   			var id = $("#editHealthInsuranceForm").find("input.healthInsuranceId").val();
   			$("#healthInsuranceContentDatagrid_edit").jqGrid({
                ...
                colModel: [{
                	...
                	label: "操作", name: "operationCell", align: "center", width: 70, formatter: function(cellvalue, options, rowObject) {
                		...
                		var rowId = options.rowId;
               			buttons = buttons + createBtn("删除", "btn-danger btn-xs", "fa-trash-o", "delEditHealthInsuranceContent(\""+rowObject.id+"\", \""+rowId+"\")");
   						return buttons;
                	}
                }],
                ...
                ondblClickRow: function(id){
                	if(id && id!==healthInsuranceLastsel2){
                		jQuery("#healthInsuranceContentDatagrid_edit").saveRow(healthInsuranceLastsel2, false, "clientArray");
                		jQuery("#healthInsuranceContentDatagrid_edit").editRow(id,true);
   			...
   			$("#healthInsuranceContentDatagrid_edit").jqGrid("setGroupHeaders", {
   			...
   			$("#delHealthInsuranceContent_edit").click(function() {
   				var selectedIds = $("#healthInsuranceContentDatagrid_edit").getGridParam("selarrrow");
   				
   				delBatchHealthInsuranceContent_edit(selectedIds);
   			});
   		}
   		 ...
   		 * 批量删除
   		 */
   		function delBatchHealthInsuranceContent_edit(ids) {
   			if(!ids && 0 >= ids.length) {
	                	...
	                	try {
		                	...
		                	for(var i = 0; i < ids.length; i ++) {
			            		...
			            		if(id && ""!=$.trim(id) && isNumber(id)) {
				            		...
				            		$("#healthInsuranceContentDatagrid_edit").delRowData(id);
				            	}
			            	}
		...
		function delEditHealthInsuranceContent(id, rowId) {
			BootstrapDialog.confirm({
	            title: "温馨提示",
	            message: "您确定删除吗？",
	            type: BootstrapDialog.TYPE_SUCCESS, // <-- Default value is BootstrapDialog.TYPE_PRIMARY
	                	...
	                	if(id && ""!=$.trim(id) && isNumber(id)) {
	                		$.ajax({
			    				...
			    				success: function(data) {
			    					...
			    					if(json.success) {
			    						BootstrapDialog.success(json.msg);
			    						$("#healthInsuranceContentDatagrid_edit").delRowData(rowId);
			    					} else {
			    						BootstrapDialog.danger(json.msg);
			    					}
			    				},
